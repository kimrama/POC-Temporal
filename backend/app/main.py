...
WORKFLOW_TASK_QUEUE = os.getenv("WORKFLOW_TASK_QUEUE", "computer-task-queue")
...
@app.get("/calculate/{initial_value}")
async def start_workflow(initial_value: int) -> StartWorkflowResponse:
    client = await get_temporal_client()

    workflow_id = f"compute-{uuid.uuid4().hex[:8]}"
